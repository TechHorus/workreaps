...
if( apply_filters('workreap_system_access','service_base') === true ){
	while ( have_posts() ) {
		...
		if(!empty($db_addons) && is_array($db_addons)){
			$db_addons	= get_posts( array('include'  			=> $db_addons,
									...
									<div class="wt-addonstwo">
										<div class="wt-addonsservices wt-tabsinfo">
											...
											<div class="wt-addonservices-content addon-list-items">
												<ul>
												<?php
													foreach( $db_addons as $key => $addon ) { 
														$db_price			= 0;
														if (function_exists('fw_get_db_post_option')) {
														...
														<li>
															<div class="wt-checkbox">
																<input data-service-id="<?php echo intval($post->ID);?>" data-addons-id="<?php echo intval($addon->ID);?>" id="rate<?php echo intval($addon->ID);?>" type="checkbox" name="addons" class="wt-addons-checkbox" value="<?php echo intval($addon->ID);?>" >
